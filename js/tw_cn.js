...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const tips_traditional = '繁' // 此處可以更改為你想要顯示的文字
  const tips_simplified = '简' // 同上，但兩處均不建議更改

  let currentEncoding = defaultEncoding
  const targetEncodingCookie = 'translate-chn-cht'
  let targetEncoding =
    localStorage.getItem(targetEncodingCookie) === null
      ? defaultEncoding
      : Number(localStorage.getItem(targetEncodingCookie))
  ...
  function translateInitialization() {
    toggle_lang = document.getElementById('translateLink');

    if (toggle_lang) {
      if (currentEncoding !== targetEncoding) {
        setTimeout(translateBody, translateDelay)

        if (targetEncoding === 1) toggle_lang.innerHTML = tips_simplified
        else toggle_lang.innerHTML = tips_traditional
      }
      toggle_lang.addEventListener('click', translatePage, false)
    }
  }
