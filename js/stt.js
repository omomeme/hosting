/* stt: simplified and traditional translation. forked from tw_cn.js*/
; (function (undefined) {
  "use strict"
  var _global;
  ...
  var stt = {
    toggle_lang: null,
    //defaultEncoding: 2, // 網站默認語言，1: 繁體中文, 2: 簡體中文
    translateDelay: 0, // 延遲時間,若不在前, 要設定延遲翻譯時間, 如100表示100ms,默認為0
    tips_traditional: '<i class="ic i-fanti"></i>',
    tips_simplified: '<i class="ic i-jianti"></i>',
    currentEncoding: 2,
    targetEncodingCookie: 'translate-chn-cht',
    targetEncoding: localStorage.getItem('translate-chn-cht') === null ? 1 : Number(localStorage.getItem('translate-chn-cht')),
    translateText: function (txt) {
      if (txt === '' || txt == null) return ''
      if (stt.currentEncoding === 1 && stt.targetEncoding === 2) return stt.Simplized(txt)
      else if (stt.currentEncoding === 2 && stt.targetEncoding === 1) { return stt.Traditionalized(txt) } else return txt
    },
    translateBody: function (fobj) {
      var objs;
      ...
      for (var i = 0; i < objs.length; i++) {
        ...
        if (obj.title !== '' && obj.title != null) { obj.title = stt.translateText(obj.title) }
        if (obj.alt !== '' && obj.alt != null) obj.alt = stt.translateText(obj.alt)
        if (obj.placeholder !== '' && obj.placeholder != null) obj.placeholder = stt.translateText(obj.placeholder)
        if (
          obj.tagName === 'INPUT' &&
          obj.value !== '' &&
          obj.type !== 'text' &&
          obj.type !== 'hidden'
        ) { obj.value = stt.translateText(obj.value) }
        if (obj.nodeType === 3) obj.data = stt.translateText(obj.data)
        else stt.translateBody(obj)
      }
    },
    translatePage: function (e) {
      if (stt.targetEncoding === 1) {
        stt.currentEncoding = 1
        stt.targetEncoding = 2
        stt.toggle_lang.innerHTML = stt.tips_traditional
        localStorage.setItem('', stt.targetEncoding)
        stt.translateBody()
      } else if (stt.targetEncoding === 2) {
        stt.currentEncoding = 2
        stt.targetEncoding = 1
        stt.toggle_lang.innerHTML = stt.tips_simplified
        localStorage.setItem('translate-chn-cht', stt.targetEncoding)
        stt.translateBody()
      }
    },
    JTPYStr: function () {
    ...
    Traditionalized: function (cc) {
      var str = ''
      const ss = stt.JTPYStr()
      const tt = stt.FTPYStr()
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    define(function () { return stt; });
  } else {
    !('stt' in _global) && (_global.stt = stt);
  }
}());